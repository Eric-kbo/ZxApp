...
Page({
    ...
    wx.uploadFile({
      ...
      success: function(res) {
        ...
        if (result.IsSuccess) {
          ...
          imgobj.id = that.data.ImageFile.length;
          imgobj.hid = false;
          imgobj.upload = "RePicture";
  ...
  RePicture: function(e) {
    const that = this;
    var imgList= new Array();
    var FileList = that.data.ImageFile;
    var countid=0;
    for (var i = 0; i < FileList.length; i++) {
      if (FileList[i].id != e.target.dataset.id)
      {
        var imgobj={};
        imgobj.src = FileList[i].src;
        imgobj.id = countid;
        imgobj.hid = false;
        imgobj.upload = FileList[i].upload != "uploadworks" ? "RePicture" :"uploadworks";
        imgList.push(imgobj);
        countid++;
      }
    }
